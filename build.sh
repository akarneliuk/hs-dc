...
sudo docker run --net=none --privileged --entrypoint /bin/bash --name leaf11 -it -d -v $PWD/leaf11:/sonic docker-sonic-p4:latest
sudo docker run --net=none --privileged --entrypoint /bin/bash --name leaf12 -it -d -v $PWD/leaf12:/sonic docker-sonic-p4:latest
sudo docker run --net=none --privileged --entrypoint /bin/bash --name spine11 -it -d -v $PWD/spine11:/sonic docker-sonic-p4:latest
...
sudo brctl addif s11_l12 veth2
sudo ip link set netns ${SPINE11} dev sw_port1
sudo ip link add sw_port5 type veth
sudo ip link set veth3 up
sudo brctl addif s11_l12 veth3
...
sudo brctl addif host12_leaf12 veth6
sudo ip link set netns ${LEAF12} dev sw_port0
sudo ip link add eth1 type veth
sudo ip link set veth7 up
sudo brctl addif host12_leaf12 veth7
